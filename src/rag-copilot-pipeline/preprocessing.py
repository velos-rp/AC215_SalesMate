...
def process_folder(
    ...
    The function takes a folder path and will extract text from all text
    and pdf files in the folder.
    It will then chunk the text according to the chunk_params and store the
    text and metadata in a pandas DataFrame.

    The metadata includes the file name and the number of chunks and the folder
    path.

    If save is set to True, the function will save the DataFrame to a single
    ...
    :param chunk_params: A dictionary of parameters for chunking, including
    the method and chunk size
    ...
    for file in files:
        ...
        # filter for pdf, if the file is a pdf then extract text from it using
        #  the helper method
        if file.suffix == ".pdf":
        ...
        embeddings = generate_text_embeddings(
            text_chunks, dimensionality=EMBEDDING_DIMENSION
        )
        ...
        df = pd.concat(
            [
                ...
                pd.DataFrame(
                    {
                        "text": text_chunks,
                        "metadata": [metadata] * len(text_chunks),
    ...
    if save:
        ...
        df.to_json(
            os.path.join(OUTPUT_FOLDER, f"{save}{timestamp}.json"), orient="records"
        )
        # also write the chunk params to a json file
        with open(
            os.path.join(OUTPUT_FOLDER, f"{save}{timestamp}_chunk_params.json"), "w"
        ) as f:
