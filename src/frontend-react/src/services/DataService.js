...
const DataService = {
    ...
    StartChatWithLLM: async function (message) {
        return await api.post((process.env.NODE_ENV === 'production' ? "/api" : "") + "/direct-chat/chats/", message);
    },
    ContinueChatWithLLM: async function (chat_id, message) {
        return await api.post((process.env.NODE_ENV === 'production' ? "/api" : "") + "/direct-chat/chats/" + chat_id, message);
    },
